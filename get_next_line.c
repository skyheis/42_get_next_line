...
/*   Updated: 2022/11/11 18:05:14 by ggiannit         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_substr_read(char **buff_read, int flag_going)
{
	int		i;
	char	*line;
	ssize_t	line_size;
...
char	*get_next_line(int fd)
{
	...
	if (ft_search_bsn(buff_read))
		line = ft_substr_read(&buff_read, flag_going);
	//else if (!flag_going && !*buff_read)
	//	return (NULL);
	else if (!flag_going)
	{
